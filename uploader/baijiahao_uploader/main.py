...
async def _grab_qr(page: Page, account_file: str) -> dict:
    ...
    baijiahao_logger.info(format_msg("🖼️", f"二维码已保存到: {qrcode_path}"))
    if qrcode_content:
        print_terminal_qrcode(qrcode_content, qrcode_path, "百度APP/手机百度")
    else:
        baijiahao_logger.warning(format_msg("😵", f"终端没法完整显示二维码，请打开 {qrcode_path} 扫码"))
...
async def baijiahao_cookie_gen(account_file, qrcode_callback=None, poll_interval: int = 3, max_checks: int = 120, headless: bool = LOCAL_CHROME_HEADLESS):
    ...
    """
    account_file = resolve_account_file(account_file, "baijiahao_uploader")
    Path(account_file).parent.mkdir(parents=True, exist_ok=True)
    qrcode_path = None
    result = build_login_result(False, "failed", "百家号登录失败", account_file)
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if headless:
                baijiahao_logger.info(format_msg("🧍", "无头登录中：二维码已存为图片，请用百度APP扫码"))
            else:
                baijiahao_logger.info(format_msg("🧍", "请在打开的浏览器中扫码登录百家号"))
            ...
            baijiahao_logger.info(format_msg("🧍", "请扫码，正在耐心等待登录完成"))

            # 轮询等待登录完成
            for _ in range(max_checks):
                if await _is_login_completed(page):
                    baijiahao_logger.info(format_msg("🥳", f"扫码成功，当前页面: {page.url}"))
                    result = build_login_result(True, "success", "百家号扫码登录成功", account_file, qrcode_info, page.url)
                    break
                await page.wait_for_timeout(poll_interval * 1000)
            else:
                result = build_login_result(False, "timeout", "等待百家号扫码登录超时", account_file, qrcode_info, page.url)

            if result["success"]:
                await asyncio.sleep(2)
                await context.storage_state(path=account_file)
                baijiahao_logger.success(format_msg("🥳", f"cookie 已保存: {account_file}"))
        except Exception as exc:
            result = build_login_result(False, "failed", str(exc), account_file, current_url=page.url if "page" in locals() else "")
        finally:
            if remove_qrcode_file(qrcode_path):
                baijiahao_logger.info(format_msg("🧹", f"临时二维码文件已清理: {qrcode_path}"))
            if not result["success"]:
                baijiahao_logger.error(format_msg("😢", f"登录失败: {result['message']}"))
            await context.close()
            await browser.close()
    return result
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if await page.get_by_text("注册/登录百家号").count():
                baijiahao_logger.info(format_msg("🥹", "cookie 已失效"))
                return False
            else:
                baijiahao_logger.success(format_msg("🥳", "cookie 有效"))
                return True
        except Exception as exc:
            baijiahao_logger.warning(format_msg("😵", f"cookie 校验出错，按失效处理: {exc}"))
            return False
        finally:
            await browser.close()
...
async def baijiahao_setup(account_file, handle=False, return_detail=False, qrcode_callback=None, headless: bool = LOCAL_CHROME_HEADLESS):
    ...
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        ...
        baijiahao_logger.info(format_msg("🥹", "cookie 文件不存在或已失效，自动打开浏览器请扫码登录"))
...
class BaiJiaHaoVideo(BaseVideoUploader):
    ...
    def __init__(
        ...
        self.tags = tags or []
        self.account_file = resolve_account_file(account_file, "baijiahao_uploader")
    ...
    async def upload(self, playwright: Playwright) -> None:
        baijiahao_logger.info(format_msg("🧍", "先检查 cookie 和视频文件"))
        await self.validate_upload_args()
        baijiahao_logger.info(format_msg("🥳", "上传前检查通过"))
        ...
        try:
            ...
            baijiahao_logger.info(format_msg("🏃", f"开始上传视频: {self.title}"))
            ...
            baijiahao_logger.info(format_msg("🏃", f"已选择视频文件: {self.file_path}"))
            ...
            baijiahao_logger.success(format_msg("🥳", "cookie 更新完毕"))
        finally:
            await context.close()
            await browser.close()
    ...
    async def _fill_title(self, page: Page) -> None:
        ...
        baijiahao_logger.info(format_msg("🏷️", f"标题已填写: {title}"))
    ...
    async def _wait_upload_complete(self, page: Page, timeout: int = 600) -> None:
        ...
        while True:
            if time.monotonic() - start > timeout:
                baijiahao_logger.warning(format_msg("⚠️", f"等待上传超时（>{timeout}s），继续后续步骤"))
                return
            ...
            if pct is not None and pct < 100:
                seen_progress = True
                gone_count = 0
                baijiahao_logger.info(format_msg("🏃", f"上传中 {pct}%"))
                await asyncio.sleep(2)
                continue
            ...
            if seen_progress:
                ...
                gone_count += 1
                if gone_count >= 2:
                    baijiahao_logger.success(format_msg("🥳", "视频上传完毕"))
                    return
                await asyncio.sleep(2)
                continue
    ...
    async def _upload_thumbnail(self, page: Page) -> None:
        ...
        try:
            ...
            baijiahao_logger.info(format_msg("🏃", "已点击「选择封面」"))
            ...
            baijiahao_logger.info(format_msg("🏃", f"已选择封面图片: {self.thumbnail_path}"))
            ...
            if confirmed:
                baijiahao_logger.success(format_msg("🖼️", "封面已上传"))
            else:
                # 没有等到确认按钮：不确定封面是否真正生效，不再冒充成功，
                # 交给下方 except 分支同一套"等待系统自动封面"兜底逻辑核实/兜底。
                raise RuntimeError("封面确认按钮未出现，无法确认封面是否生效")
        except Exception as exc:
            baijiahao_logger.warning(format_msg("⚠️", f"封面上传失败: {exc}，尝试等待系统自动封面"))
            # fallback：等系统自动生成
            await self._wait_cover_ready(page)
    ...
    async def _check_ai_declaration(self, page: Page) -> None:
        ...
        try:
            ...
            baijiahao_logger.success(format_msg("🏷️", "已选择「含AI生成内容」"))
        except Exception as exc:
            # 如果失败，尝试关闭可能残留的弹窗
            try:
                await page.keyboard.press("Escape")
                ...
                pass
            baijiahao_logger.warning(format_msg("⚠️", f"选择 AI 声明失败: {exc}"))
    ...
    async def _apply_collection(self, page: Page) -> None:
        ...
        try:
            ...
            if not await select_box.count():
                baijiahao_logger.warning(format_msg("⚠️", "未找到合集选择器，跳过"))
                return
            ...
            if await option.count():
                ...
                await page.wait_for_timeout(500)
                baijiahao_logger.success(format_msg("🥳", f"已选择合集：{self.collection_name}"))
            else:
                baijiahao_logger.warning(format_msg("⚠️", f"账号中无「{self.collection_name}」合集，跳过"))
                await page.keyboard.press("Escape")
        except Exception as exc:
            baijiahao_logger.warning(format_msg("⚠️", f"选择合集失败，跳过: {exc}"))
    ...
    async def _wait_cover_ready(self, page: Page, timeout: int = 120) -> None:
        ...
        while True:
            if time.monotonic() - start > timeout:
                baijiahao_logger.warning(format_msg("⚠️", "等待封面生成超时，继续发布"))
                return
            if await page.locator("div.cheetah-spin-container img").count():
                baijiahao_logger.info(format_msg("🖼️", "封面已生成"))
                return
            baijiahao_logger.info(format_msg("🏃", "等待封面生成..."))
    ...
    async def _submit_publish(self, page: Page) -> None:
        ...
        baijiahao_logger.info(format_msg("🏃", "已点击发布按钮"))
        ...
        while time.monotonic() - start < 30:
            ...
            if BAIJIAHAO_SUCCESS_URL_PREFIX in url or "/rc/content" in url or "/rc/home" in url:
                baijiahao_logger.success(format_msg("🥳", "视频发布成功"))
                return
            # 检查是否出现百度安全验证
            ...
            if await error_toast.count() and await error_toast.is_visible():
                ...
                baijiahao_logger.warning(format_msg("⚠️", f"发布提示: {err_text}"))
            await page.wait_for_timeout(1000)
