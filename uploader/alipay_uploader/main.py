...
async def _capture_alipay_qr(page: Page, account_file: str, previous_qrcode_path: Path | None = None) -> dict:
    ...
    if previous_qrcode_path and previous_qrcode_path != qrcode_path:
        if remove_qrcode_file(previous_qrcode_path):
            alipay_logger.info(format_msg("🧹", f"临时二维码文件已清理: {previous_qrcode_path}"))
    alipay_logger.info(format_msg("🖼️", f"二维码已保存到: {qrcode_path}"))
    if qrcode_content:
        print_terminal_qrcode(qrcode_content, qrcode_path, "支付宝APP")
    else:
        alipay_logger.warning(format_msg("😵", f"终端没法完整显示二维码，请打开 {qrcode_path} 扫码"))
...
async def alipay_cookie_gen(account_file, qrcode_callback=None, poll_interval: int = 3, max_checks: int = 100, headless: bool = LOCAL_CHROME_HEADLESS):
    ...
    """
    account_file = resolve_account_file(account_file, "alipay_uploader")
    Path(account_file).parent.mkdir(parents=True, exist_ok=True)
    qrcode_path = None
    result = build_login_result(False, "failed", "支付宝登录失败", account_file)
    async with async_playwright() as playwright:
        browser = await playwright.chromium.launch(**build_launch_kwargs(headless=headless))
        ...
        try:
            ...
            if headless:
                alipay_logger.info(format_msg("🧍", "无头登录中：二维码已存为图片，请在终端扫或打开图片扫码（或由发布进程推送到飞书）"))
            else:
                alipay_logger.info(format_msg("🧍", "请在打开的浏览器中扫码登录支付宝生活号（登录完成后请勿手动关闭浏览器）"))
            ...
            for _ in range(30):
                ...
                await context.close()
                await browser.close()
                return build_login_result(False, "timeout", "等待登录弹窗超时（30s），放弃保存", account_file, current_url=page.url)
            ...
            alipay_logger.info(format_msg("🧍", "请扫码，正在耐心等待登录完成"))
            ...
            for _i in range(max_checks):  # 最多等 3~5 分钟
                if await _is_alipay_login_completed(page, login_iframe):
                    alipay_logger.info(format_msg("🥳", f"扫码成功，已经跳转到登录后页面: {page.url}"))
                    result = build_login_result(True, "success", "支付宝扫码登录成功", account_file, qrcode_info, page.url)
                    break
                await page.wait_for_timeout(poll_interval * 1000)
            else:
                result = build_login_result(False, "timeout", "等待支付宝扫码登录超时", account_file, qrcode_info, page.url)
            ...
            if result["success"]:
                ...
                try:
                    ...
                    if not _has_cookie:
                        result = build_login_result(False, "cookie_invalid", "支付宝扫码流程结束，但 cookie 为空", account_file, qrcode_info, page.url)
                except Exception as _e:
                    alipay_logger.warning(format_msg("⚠️", f"cookie 文件校验异常（忽略，按成功处理）: {_e}"))
        except Exception as exc:
            result = build_login_result(False, "failed", str(exc), account_file, current_url=page.url if "page" in locals() else "")
        finally:
            if remove_qrcode_file(qrcode_path):
                alipay_logger.info(format_msg("🧹", f"临时二维码文件已清理: {qrcode_path}"))
            if not result["success"]:
                alipay_logger.error(format_msg("😢", f"登录失败: {result['message']}"))
            await context.close()
            await browser.close()
    return result
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if url.startswith("https://auth.alipay.com/") or "login" in url.lower():
                alipay_logger.info(format_msg("🥹", "cookie 已失效（跳转到登录页）"))
                return False
            if await page.locator('iframe[title="login"]').count():
                alipay_logger.info(format_msg("🥹", "cookie 已失效（出现登录弹窗）"))
                return False

            alipay_logger.success(format_msg("🥳", "cookie 有效"))
            return True
        except Exception as exc:
            alipay_logger.warning(format_msg("😵", f"cookie 校验时出错，按失效处理: {exc}"))
            return False
        finally:
            await browser.close()
...
async def alipay_setup(account_file, handle=False, return_detail=False, qrcode_callback=None, headless: bool = LOCAL_CHROME_HEADLESS):
    ...
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        if not handle:
            result = build_login_result(False, "cookie_invalid", "cookie 文件不存在或已失效", account_file)
            return result if return_detail else False
        alipay_logger.info(format_msg("🥹", "cookie 文件不存在或已失效，自动打开浏览器请扫码登录"))
        result = await alipay_cookie_gen(account_file, qrcode_callback=qrcode_callback, headless=headless)
        return result if return_detail else result["success"]

    result = build_login_result(True, "cookie_valid", "cookie 有效", account_file)
...
class AlipayVideo(BaseVideoUploader):
    def __init__(
        ...
        self.tags = tags or []
        self.account_file = resolve_account_file(account_file, "alipay_uploader")
        self.desc = desc or ""
    ...
    async def open_upload_page(self, page: Page) -> None:
        ...
        try:
            ...
            alipay_logger.warning(format_msg("😵", f"点击「发布视频」入口失败: {exc}"))
            raise
    ...
    async def upload_video_file(self, page: Page, file_path: str) -> None:
        ...
        alipay_logger.info(format_msg("🏃", f"已选择视频文件: {file_path}"))
    ...
    async def fill_title_and_tags(self, page: Page) -> None:
        ...
        alipay_logger.info(format_msg("🏷️", f"标题已填写（含标签共 {len(value)} 字）: {value}"))
    ...
    async def fill_description(self, page: Page) -> None:
        ...
        alipay_logger.info(format_msg("🏷️", f"作品描述已填写: {self.desc[:40]}"))
    ...
    async def upload_thumbnail(self, page: Page) -> None:
        ...
        try:
            ...
            alipay_logger.success(format_msg("🖼️", "封面已上传"))
        except Exception as exc:
            alipay_logger.warning(format_msg("😵", f"封面上传失败，跳过继续: {exc}"))
    ...
    async def apply_collection(self, page: Page) -> None:
        ...
        try:
            ...
            if await target.count():
                ...
                await page.wait_for_timeout(800)
                alipay_logger.success(format_msg("🥳", f"已选择合集：{self.collection_name}"))
                return

            alipay_logger.warning(format_msg("😵", f"账号中无「{self.collection_name}」合集，跳过归集"))
            await page.keyboard.press("Escape")
        except Exception as exc:
            alipay_logger.warning(format_msg("😵", f"选择合集失败，跳过归集继续发布: {exc}"))
    ...
    async def check_ai_label(self, page: Page) -> None:
        ...
        try:
            if await ai_label.count():
                ...
                alipay_logger.success(format_msg("🏷️", "已勾选「内容由AI生成」"))
        except Exception as exc:
            alipay_logger.warning(format_msg("😵", f"勾选「内容由AI生成」失败: {exc}"))
        ...
        while True:
            ...
            try:
                ...
                if await publish_btn.is_disabled():
                    alipay_logger.info(format_msg("🏃", "正在上传/转码视频中..."))
                    await asyncio.sleep(2)
                    continue
                alipay_logger.success(format_msg("🥳", "视频上传完毕"))
                return
            except Exception:
                alipay_logger.info(format_msg("🏃", "正在上传/转码视频中..."))
                await asyncio.sleep(2)
    ...
    async def submit_publish(self, page: Page) -> None:
        ...
        async def _collect_click_diag(tag: str):
            try:
                ...
                alipay_logger.info(format_msg("🔍", f"{tag}: {_json.dumps(diag, ensure_ascii=False)[:1000]}"))
            except Exception as exc:
                alipay_logger.warning(format_msg("🔍", f"{tag}: 诊断采集失败: {exc}"))
        ...
        async def _dismiss_quality_modal() -> bool:
            ...
            try:
                ...
                if await btn.count() and await btn.is_visible():
                    ...
                    alipay_logger.info(format_msg("✅", "已点击优化项弹窗「继续发布」放行提交"))
                    await page.wait_for_timeout(500)
                    return True
            except Exception as exc:
                alipay_logger.warning(format_msg("😵", f"点击「继续发布」失败: {exc}"))
            return False
        ...
        try:
            while time.monotonic() - start <= timeout:
                if "/content-creation/posts" in page.url:
                    alipay_logger.success(format_msg("🥳", "视频发布成功"))
                    return
                ...
                try:
                    ...
                    if saw_aigc_done and not retried_after_aigc:
                        if await publish_btn.count() and not await publish_btn.is_disabled():
                            ...
                            alipay_logger.info(format_msg("🔁", "检测到 AIGC 预处理完成，已二次点击确认发布"))
                            await page.wait_for_timeout(500)
                ...
                if no_submit_signal and submit_click_retries < 3 and (time.monotonic() - last_click_ts) >= 8:
                    try:
                        if await publish_btn.count() and not await publish_btn.is_disabled():
                            ...
                            alipay_logger.info(format_msg("🔁", f"未检测到提交请求信号，重试点击确认发布（{submit_click_retries}/3）"))
                            await page.wait_for_timeout(500)
                ...
                try:
                    if await success_toast.count() and await success_toast.is_visible():
                        alipay_logger.success(format_msg("🥳", "视频发布成功（toast 命中）"))
                        return
                except Exception:
                    pass
                try:
            ...
            if net_tasks:
                try:
                    ...
                    pass
            if req_events:
                alipay_logger.info(format_msg("🧾", f"提交阶段请求事件 {len(req_events)} 条（最近10条）"))
                for m, u, b in req_events[-10:]:
                    alipay_logger.info(format_msg("🧾", f"REQ {m} {u} | {b}"))
            if net_events:
                alipay_logger.info(format_msg("🧾", f"提交阶段网络事件 {len(net_events)} 条（最近10条）"))
                for m, u, s, b in net_events[-10:]:
                    alipay_logger.info(format_msg("🧾", f"{m} {s} {u} | {b}"))

    async def upload(self, playwright: Playwright) -> None:
        alipay_logger.info(format_msg("🧍", "先检查 cookie 和视频文件"))
        await self.validate_upload_args()
        alipay_logger.info(format_msg("🥳", "上传前检查通过"))

        browser = await playwright.chromium.launch(**build_launch_kwargs(headless=self.headless))
        ...
        try:
            ...
            alipay_logger.info(format_msg("🏃", f"开始上传视频: {self.title}"))
            ...
            alipay_logger.success(format_msg("🥳", "cookie 更新完毕"))
        finally:
            await context.close()
            await browser.close()
