...
async def hupu_cookie_gen(account_file, qrcode_callback=None, poll_interval: int = 3, max_checks: int = 120, headless: bool = False):
    ...
    result = build_login_result(False, "failed", "虎扑登录失败", account_file)

    async with async_playwright() as playwright:
        browser = await playwright.chromium.launch(**build_launch_kwargs(headless=headless, extra_args=["--disable-blink-features=AutomationControlled"]))
        ...
        try:
            ...
            if not await qq_btn.count():
                hupu_logger.error(format_msg("😢", "未找到 QQ 登录按钮"))
                result = build_login_result(False, "failed", "未找到 QQ 登录按钮", account_file, current_url=page.url)
                return result

            await qq_btn.click()
            await page.wait_for_timeout(5000)
            hupu_logger.info(format_msg("🏃", "已跳转到 QQ 登录页"))
            ...
            if qrcode_info:
                ...
                hupu_logger.info(format_msg("🧍", "请用 QQ 手机版扫码登录"))
            else:
                hupu_logger.warning(format_msg("⚠️", "未能获取 QQ 二维码，请在浏览器中手动扫码"))
            ...
            for _ in range(max_checks):
                ...
                if "www.hupu.com" in current_url and "passport" not in current_url and "graph.qq.com" not in current_url:
                    hupu_logger.info(format_msg("🥳", f"登录成功，跳转到: {current_url}"))
                    result = build_login_result(True, "success", "虎扑 QQ 扫码登录成功", account_file, current_url=current_url)
                    break
                # 检查是否实际跳转到虎扑 passport 回调页（不是 redirect_uri 参数中包含）
                if current_url.startswith("https://passport.hupu.com/pc/qqcallback"):
                    await page.wait_for_timeout(5000)
                    current_url = page.url
                    hupu_logger.info(format_msg("🥳", f"QQ 回调成功，当前: {current_url}"))
                    result = build_login_result(True, "success", "虎扑 QQ 扫码登录成功", account_file, current_url=current_url)
                    break
                # 检查 cookies 中是否出现 u（核心登录态）
                cookies = await context.cookies()
                if any(c.get("name") == "u" and c.get("value") for c in cookies):
                    hupu_logger.info(format_msg("🥳", f"登录成功（检测到 u cookie），当前: {current_url}"))
                    result = build_login_result(True, "success", "虎扑 QQ 扫码登录成功", account_file, current_url=current_url)
                    break
                await page.wait_for_timeout(poll_interval * 1000)
            else:
                result = build_login_result(False, "timeout", "等待 QQ 扫码登录超时", account_file, current_url=page.url)
            ...
            if result["success"]:
                ...
                hupu_logger.success(format_msg("🥳", f"cookie 已保存: {account_file}"))
            ...
            if not result["success"]:
                hupu_logger.error(format_msg("😢", f"登录失败: {result['message']}"))
            await context.close()
            await browser.close()
    return result
            ...
            if src and src.startswith("http"):
                ...
                try:
                    ...
                    if qrcode_content:
                        ...
                        hupu_logger.warning(format_msg("😵", f"终端无法显示二维码，请打开 {qr_path} 扫码"))

                    return {"image_path": str(qr_path), "image_data_url": ""}
                except Exception as exc:
                    hupu_logger.warning(format_msg("⚠️", f"下载 QQ 二维码失败: {exc}"))
                    continue
...
async def cookie_auth(account_file):
    ...
    account_file = _resolve_account_file(account_file)
    async with async_playwright() as playwright:
        browser = await playwright.chromium.launch(**build_launch_kwargs(headless=True, extra_args=["--disable-blink-features=AutomationControlled"]))
        try:
            ...
            if "passport" in page.url or "login" in page.url:
                hupu_logger.info(format_msg("🥹", "cookie 已失效（跳转到登录页）"))
                return False

            # 检查发布页是否出现「上传视频」按钮（登录后才有）
            upload_btn = page.get_by_role("button", name="上传视频")
            if await upload_btn.count():
                hupu_logger.success(format_msg("🥳", "cookie 有效"))
                return True

            # 兜底：检查 cookie 中是否有 u 字段
            cookies = await context.cookies()
            if any(c.get("name") == "u" and c.get("value") for c in cookies):
                hupu_logger.success(format_msg("🥳", "cookie 有效（u cookie 存在）"))
                return True

            hupu_logger.info(format_msg("🥹", "cookie 已失效（未检测到登录态）"))
            return False
        except Exception as exc:
            hupu_logger.warning(format_msg("😵", f"cookie 校验出错，按失效处理: {exc}"))
            return False
        finally:
            await browser.close()
...
async def hupu_setup(account_file, handle=False, return_detail=False, qrcode_callback=None, headless: bool = False):
    ...
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        ...
        hupu_logger.info(format_msg("🥹", "cookie 文件不存在或已失效，打开浏览器请手动登录"))
...
class HuPuVideo(BaseVideoUploader):
    ...
    async def upload(self, playwright: Playwright) -> None:
        hupu_logger.info(format_msg("🧍", "先检查 cookie 和视频文件"))
        await self.validate_upload_args()
        hupu_logger.info(format_msg("🥳", "上传前检查通过"))

        browser = await playwright.chromium.launch(**build_launch_kwargs(headless=self.headless, extra_args=["--disable-blink-features=AutomationControlled"]))
        ...
        try:
            ...
            await page.wait_for_timeout(5000)
            hupu_logger.info(format_msg("🏃", f"开始上传视频: {self.title}"))
            ...
            hupu_logger.success(format_msg("🥳", "cookie 更新完毕"))
        finally:
            await context.close()
            await browser.close()
    ...
    async def _upload_video_file(self, page: Page) -> None:
        ...
        hupu_logger.info(format_msg("🏃", f"已选择视频文件: {self.file_path}"))
        ...
        hupu_logger.info(format_msg("🥳", "视频已就绪"))
    ...
    async def _fill_description(self, page: Page) -> None:
        ...
        if content:
            ...
            hupu_logger.info(format_msg("📝", f"简介已填写（{len(content)}字）"))
    ...
    async def _upload_thumbnail(self, page: Page) -> None:
        ...
        try:
            ...
            hupu_logger.info(format_msg("🏃", f"已选择封面图片: {self.thumbnail_path}"))
            await page.wait_for_timeout(3000)
            hupu_logger.success(format_msg("🖼️", "封面已上传"))
        except Exception as exc:
            hupu_logger.warning(format_msg("⚠️", f"封面上传失败: {exc}，继续发布（使用默认封面）"))
    ...
    async def _select_zone(self, page: Page) -> None:
        ...
        try:
            ...
            await page.wait_for_timeout(1000)
            hupu_logger.info(format_msg("🏷️", "已选择专区：步行街主干道"))
        except Exception as exc:
            hupu_logger.warning(format_msg("⚠️", f"选择专区失败: {exc}"))
    ...
    async def _check_declarations(self, page: Page) -> None:
        ...
        try:
            ...
            if await declaration_btn.count():
                ...
                await page.wait_for_timeout(1000)
                hupu_logger.info(format_msg("🏷️", "已点击「原创/二创」"))
        except Exception as exc:
            hupu_logger.warning(format_msg("⚠️", f"点击原创/二创失败: {exc}"))
        ...
        try:
            ...
            if await combobox.count():
                ...
                if await ai_option.count():
                    ...
                    await page.wait_for_timeout(500)
                    hupu_logger.info(format_msg("🏷️", "已选择「含AI生成内容」"))
        except Exception as exc:
            hupu_logger.warning(format_msg("⚠️", f"选择 AI 声明失败: {exc}"))
    ...
    async def _submit_publish(self, page: Page) -> None:
        ...
        hupu_logger.info(format_msg("🏃", "已点击「确定发布」"))
        ...
        while time.monotonic() - start < 60:
            ...
            if HUPU_POST_URL_PATTERN.search(current_url):
                hupu_logger.success(format_msg("🥳", f"视频发布成功: {current_url}"))
                return

            await page.wait_for_timeout(2000)

        # 超时 - 可能已成功但检测不到
        hupu_logger.warning(format_msg("⚠️", f"发布后 60s 未检测到帖子页面跳转，当前 URL: {page.url}"))
