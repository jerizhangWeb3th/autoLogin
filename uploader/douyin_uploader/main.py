...
from uploader.base_login import build_login_result, emit_qrcode_callback, format_msg
...
async def douyin_setup(account_file, handle=False, return_detail=False, qrcode_callback=None, headless: bool = LOCAL_CHROME_HEADLESS, cdp_url: str | None = None):
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        if not handle:
            result = build_login_result(False, "cookie_invalid", "cookie文件不存在或已失效", account_file)
            return result if return_detail else False
        douyin_logger.info(format_msg("🥹", "cookie 失效了，准备打开浏览器重新登录"))
        result = await douyin_cookie_gen(account_file, qrcode_callback=qrcode_callback, headless=headless, cdp_url=cdp_url)
        return result if return_detail else result["success"]

    result = build_login_result(True, "cookie_valid", "cookie有效", account_file)
...
async def _save_douyin_qrcode(page: Page, account_file: str, previous_qrcode_path: Path | None = None, qrcode_callback=None) -> dict:
    ...
    try:
        ...
        douyin_logger.warning(format_msg("😵", f"没定位到二维码元素（{str(exc)[:50]}）——请直接在弹出的浏览器里扫码，小人继续等登录跳转"))
    ...
    if previous_qrcode_path and previous_qrcode_path != qrcode_path:
        if remove_qrcode_file(previous_qrcode_path):
            douyin_logger.info(format_msg("🧹", f"临时二维码文件已清理: {previous_qrcode_path}"))
    douyin_logger.info(format_msg("🖼️", f"二维码已经准备好啦，已保存到: {qrcode_path}"))
    qrcode_content = decode_qrcode_from_path(qrcode_path)
    if qrcode_content:
        print_terminal_qrcode(qrcode_content, qrcode_path, "抖音APP")
    else:
        douyin_logger.warning(format_msg("😵", f"终端没法完整显示二维码，请打开 {qrcode_path} 扫码"))
    qrcode_info = {
        "image_path": str(qrcode_path),
        "image_data_url": qrcode_src,
    }
    await emit_qrcode_callback(qrcode_callback, qrcode_info)
    return qrcode_info
...
async def _wait_for_douyin_login(page: Page, account_file: str, qrcode_info: dict, qrcode_callback=None, poll_interval: int = 3, max_checks: int = 100) -> dict:
    ...
    for _ in range(max_checks):
        if await _is_douyin_login_completed(page):
            douyin_logger.info(format_msg("🥳", f"扫码成功，已经跳转到登录后页面: {page.url}"))
            return build_login_result(True, "success", "抖音扫码登录成功", account_file, qrcode_info, page.url)
        ...
        if page.url != original_url and not await _is_douyin_login_completed(page):
            ...
            if await sms_input.count() > 0:
                if not saw_2fa:
                    douyin_logger.warning(format_msg("⚠️", f"检测到抖音短信/安全二次验证，请在弹出的浏览器中手动输入。等待 sessionid ({_}/{max_checks})"))
                    saw_2fa = True
        ...
        if await expired_box.count() and await expired_box.is_visible():
            douyin_logger.warning(format_msg("😵", "二维码失效了，小人马上去刷新"))
    ...
    return build_login_result(False, "timeout", "等待抖音扫码登录超时", account_file, qrcode_info, page.url)
...
async def douyin_cookie_gen(
    ...
    async with async_playwright() as playwright:
        ...
        context = await set_init_script(context)
        qrcode_path = None
        result = build_login_result(False, "failed", "抖音登录失败", account_file)
        try:
            page = await context.new_page()
            await page.goto("https://creator.douyin.com/")
            qrcode_info = await _save_douyin_qrcode(page, account_file, qrcode_callback=qrcode_callback)
            qrcode_path = Path(qrcode_info["image_path"]) if qrcode_info.get("image_path") else None
            douyin_logger.info(format_msg("🧍", "请扫码，小人正在耐心等待登录完成"))
            ...
            if result["success"]:
                ...
                try:
                    ...
                    douyin_logger.warning(format_msg("⚠️", f"cookie 文件校验异常（忽略，按成功处理）: {_e}"))
        except Exception as exc:
            result = build_login_result(False, "failed", str(exc), account_file, current_url=page.url if "page" in locals() else "")
        finally:
            if remove_qrcode_file(qrcode_path):
                douyin_logger.info(format_msg("🧹", f"临时二维码文件已清理: {qrcode_path}"))
            if not result["success"]:
                douyin_logger.error(format_msg("😢", f"登录失败: {result['message']}"))
...
class DouYinBaseUploader(BaseVideoUploader):
    ...
    async def handle_product_dialog(self, page: Page, product_title: str):
        ...
        if not await short_title_input.count():
            douyin_logger.error(format_msg("😵", "没找到商品短标题输入框"))
            return False
        ...
        if "disabled" not in await finish_button.get_attribute("class"):
            ...
            douyin_logger.debug(format_msg("🥳", "已点击“完成编辑”按钮"))
            await page.wait_for_selector(".semi-modal-content", state="hidden", timeout=5000)
            return True

        douyin_logger.error(format_msg("😵", "“完成编辑”按钮是灰的，小人先把弹窗关掉"))
    ...
    async def set_product_link(self, page: Page, product_link: str, product_title: str):
        ...
        try:
            ...
            if not await dropdown.count():
                douyin_logger.error(format_msg("😵", "没找到标签下拉框"))
                return False
            douyin_logger.debug(format_msg("🧍", "找到标签下拉框，小人准备选择“购物车”"))
            await dropdown.click()
            await page.wait_for_selector('[role="listbox"]', timeout=5000)
            await page.locator('[role="option"]:has-text("购物车")').click()
            douyin_logger.debug(format_msg("🥳", "已经选中“购物车”"))
            ...
            douyin_logger.debug(format_msg("🔗", f"商品链接已经填好了: {product_link}"))

            add_button = page.locator('span:has-text("添加链接")')
            button_class = await add_button.get_attribute("class")
            if "disable" in button_class:
                douyin_logger.error(format_msg("😵", "“添加链接”按钮现在点不了"))
                return False
            await add_button.click()
            douyin_logger.debug(format_msg("🥳", "已点击“添加链接”按钮"))
            ...
            if await error_modal.count():
                ...
                douyin_logger.error(format_msg("😢", "这个商品链接无效"))
                return False

            if not await self.handle_product_dialog(page, product_title):
                return False

            douyin_logger.debug(format_msg("🥳", "商品链接设置好了"))
            return True
        except Exception as e:
            douyin_logger.error(format_msg("😢", f"设置商品链接时出错: {str(e)}"))
            return False
    ...
    async def set_self_declaration(self, page: Page, declaration: str) -> bool:
        ...
        try:
            ...
            if await dialog.count() == 0:
                douyin_logger.warning(format_msg("🧾", "自主声明弹窗未打开，跳过声明继续发布"))
                return False
            ...
            try:
                ...
                pass
            douyin_logger.success(format_msg("🧾", f"自主声明已选择「{declaration}」"))
            return True
        except Exception as exc:
            douyin_logger.warning(format_msg("🧾", f"自主声明设置失败，跳过该步骤继续发布：{exc}"))
            return False
    ...
    async def select_bgm(self, page: Page, bgm_name: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                douyin_logger.warning(format_msg("🎵", f"音乐「{bgm_name}」搜索结果为空，小人跳过"))
            ...
            try:
                ...
                if await song_name_el.count():
                    ...
                    douyin_logger.info(format_msg("🎵", f"小人找到了: {song_name}"))
            except Exception:
                pass
            ...
            douyin_logger.info(format_msg("🥳", f"BGM「{bgm_name}」已应用"))
            ...
            douyin_logger.warning(format_msg("🎵", f"添加 BGM 时出错，跳过该步骤继续发布：{exc}"))
            try:
                await self._close_music_sidesheet(page)
            except Exception:
                pass
...
class DouYinVideo(DouYinBaseUploader):
    ...
    async def apply_collection(self, page: Page) -> None:
        ...
        try:
            ...
            if await trigger.count() == 0:
                douyin_logger.warning(format_msg("😵", "未找到\"添加合集\"下拉框，跳过归集"))
                return
            ...
            if await option.count() == 0:
                douyin_logger.warning(
                    format_msg("😵", f"合集下拉框未找到「{self.collection_name}」，跳过归集，保持未选状态")
                )
                await page.keyboard.press("Escape")
            ...
            douyin_logger.success(format_msg("🥳", f"已选择合集：{self.collection_name}"))
        except Exception as exc:
            douyin_logger.warning(format_msg("😵", f"选择合集失败，跳过归集继续发布: {exc}"))
            try:
                await page.keyboard.press("Escape")
            except Exception:
                pass
    ...
    async def _submit_sms_verify_code(self, page: Page, sms_input, code: str, code_file: str) -> bool:
        douyin_logger.info(format_msg("✍️", f"已获取验证码，准备填入: {code}"))
        await sms_input.click()
        await sms_input.fill(code)
        douyin_logger.info(format_msg("✅", "验证码已填入输入框"))
        await page.wait_for_timeout(500)
        ...
        if await verify_btn.count() and await verify_btn.is_visible():
            try:
                ...
                douyin_logger.success(format_msg("✅", "已点击「验证」按钮(force)"))
            except Exception:
                await page.eval_on_selector('div.uc-ui-verify_sms-verify_button', 'el => el.click()')
                douyin_logger.success(format_msg("✅", "已点击「验证」按钮(JS)"))
        else:
            verify_by_text = page.get_by_text("验证", exact=True).first
            if await verify_by_text.count():
                await verify_by_text.click(force=True)
                douyin_logger.success(format_msg("✅", "已点击「验证」按钮(text)"))
            else:
                douyin_logger.warning(format_msg("⚠️", "未找到验证按钮，尝试按Enter"))
                await page.keyboard.press("Enter")

        if os.path.exists(code_file):
            os.remove(code_file)
            douyin_logger.info(format_msg("🧹", "验证码文件已清理"))

        await page.wait_for_timeout(3000)
        douyin_logger.info(format_msg("🔄", "验证码处理完成，继续发布流程"))
        return True
    ...
    async def handle_upload_error(self, page):
        douyin_logger.warning(format_msg("😵", "视频上传摔了一跤，小人马上重新上传"))
        await page.locator('div.progress-div [class^="upload-btn-input"]').set_input_files(self.file_path)

    async def handle_auto_video_cover(self, page):
        if await page.get_by_text("请设置封面后再发布").first.is_visible():
            douyin_logger.info(format_msg("🧍", "发布前还得先把封面弄好"))
            recommend_cover = page.locator('[class^="recommendCover-"]').first
            if await recommend_cover.count():
                douyin_logger.info(format_msg("🏃", "小人去选第一个推荐封面"))
                try:
                    await recommend_cover.click()
                    await asyncio.sleep(1)
                    confirm_text = "是否确认应用此封面？"
                    if await page.get_by_text(confirm_text).first.is_visible():
                        douyin_logger.info(format_msg("🪟", f"弹出确认框了: {confirm_text}"))
                        await page.get_by_role("button", name="确定").click()
                        douyin_logger.info(format_msg("🥳", "推荐封面已经应用"))
                        await asyncio.sleep(1)
                    douyin_logger.info(format_msg("🥳", "封面选择流程完成"))
                    return True
                except Exception as e:
                    douyin_logger.warning(format_msg("😵", f"推荐封面没选成功: {e}"))
        return False
    ...
    async def set_thumbnail(self, page: Page):
        ...
        douyin_logger.info(format_msg("🏃", "小人正在设置视频封面"))
        ...
        for attempt in range(5):
            ...
            douyin_logger.info(format_msg("🖼️", f"已点「{trigger_txt}」尝试打开封面弹窗(第{attempt + 1}次)"))
            try:
                ...
                continue
        if not opened:
            douyin_logger.warning(format_msg("⚠️", "封面弹窗打不开，跳过自定义封面继续发布（交给推荐封面兜底）"))
            return
        ...
        if self.thumbnail_portrait_path:
            ...
            await page.wait_for_timeout(3000)
            douyin_logger.info(format_msg("🖼️", "竖版封面已上传到预览"))
            ...
            await page.wait_for_timeout(3000)
            douyin_logger.info(format_msg("🖼️", "横版封面已上传到预览"))
        ...
        for attempt in range(4):
            ...
            # 仍没关掉：Esc 兜底后再验证一次
            douyin_logger.debug(format_msg("🖼️", f"封面「完成」后弹窗未关，重试(第{attempt + 1}次)"))
            await page.keyboard.press("Escape")
            await page.wait_for_timeout(1000)
            if await cover_locator.count() == 0:
                closed = True
        ...
        if closed:
            douyin_logger.info(format_msg("🥳", "视频封面设置完成，弹窗已关闭"))
        else:
            douyin_logger.warning(format_msg("⚠️", "封面弹窗未能关闭，可能挡住自主声明/发布"))


    async def upload(self, playwright: Playwright) -> None:
        douyin_logger.info(format_msg("🧍", "小人先检查 cookie、视频文件、封面和发布时间"))
        await self.validate_upload_args()
        douyin_logger.info(format_msg("🥳", "上传前检查通过"))
        ...
        page = await context.new_page()
        await page.goto("https://creator.douyin.com/creator-micro/content/upload", wait_until="domcontentloaded", timeout=90000)
        douyin_logger.info(format_msg("🏃", f"小人开始搬运视频: {self.title}.mp4"))
        douyin_logger.info(format_msg("🧭", "小人正在赶往上传主页"))
        ...
        while True:
            try:
                ...
                douyin_logger.info(format_msg("🥳", "已经进入 version_1 发布页面"))
                break
            except Exception:
                try:
                    await page.wait_for_url(
                    ...
                    douyin_logger.info(format_msg("🥳", "已经进入 version_2 发布页面"))
                    break
                except Exception:
                    douyin_logger.debug(format_msg("🧍", "还没进到视频发布页面，小人继续等一会"))
                    await asyncio.sleep(0.5)

        await asyncio.sleep(1)
        douyin_logger.info(format_msg("✍️", "小人开始填标题、描述和话题"))
        await self.fill_title_and_description(page, self.title, self.desc, self.tags)
        douyin_logger.info(format_msg("🏷️", f"小人一共贴了 {len(self.tags)} 个话题"))

        while True:
            try:
                number = await page.locator('[class^="long-card"] div:has-text("重新上传")').count()
                if number > 0:
                    douyin_logger.success(format_msg("🥳", "视频已经传完啦"))
                    break
                douyin_logger.info(format_msg("🏃", "小人正在努力上传视频"))
                await asyncio.sleep(2)
                if await page.locator('div.progress-div > div:has-text("上传失败")').count():
                    douyin_logger.error(format_msg("😵", "检测到上传失败，小人准备重试"))
                    await self.handle_upload_error(page)
            except Exception:
                douyin_logger.debug(format_msg("🧍", "小人还在等视频上传完成"))
                await asyncio.sleep(2)

        if self.productLink and self.productTitle:
            douyin_logger.info(format_msg("🛒", "小人正在设置商品链接"))
            await self.set_product_link(page, self.productLink, self.productTitle)
            douyin_logger.info(format_msg("🥳", "商品链接设置完成"))
        ...
        while True:
            try:
                ...
                if await sms_input.count() and await sms_input.is_visible():
                    douyin_logger.warning(format_msg("📱", "检测到短信验证码弹窗"))
                    # 点击「获取验证码」按钮（仅首次）
                    ...
                    if await get_code_btn.count() and await get_code_btn.is_visible():
                        ...
                        douyin_logger.info(format_msg("📤", "已点击「获取验证码」，请查看手机短信"))
                    ...
                    if code:
                        ...
                        douyin_logger.warning(format_msg("⏳", f"等待验证码输入；可在交互终端直接输入，或写入文件: {code_file}"))
                ...
                douyin_logger.success(format_msg("🥳", "视频发布成功，小人开心收工"))
                break
            except Exception:
                await self.handle_auto_video_cover(page)
                douyin_logger.info(format_msg("🏃", "小人正在冲刺发布视频"))
                if self.debug:
                    await page.screenshot(full_page=True)
                await asyncio.sleep(0.5)

        await context.storage_state(path=self.account_file)
        douyin_logger.success(format_msg("🥳", "cookie 更新完毕"))
        await asyncio.sleep(2)
        await context.close()
        await browser.close()
...
class DouYinNote(DouYinBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        douyin_logger.info(format_msg("🏃", f"小人开始搬运图文，共 {len(self.image_paths)} 张图片"))
        douyin_logger.info(format_msg("🔀", "小人正在切换到图文发布"))
        await page.get_by_text("发布图文", exact=True).click()
        await page.wait_for_timeout(1000)

        douyin_logger.info(format_msg("📤", "小人正在上传图片"))
        ...
        while True:
            try:
                ...
                douyin_logger.info(format_msg("🥳", "已经进入图文发布页面"))
                break
            except Exception:
                douyin_logger.debug(format_msg("🧍", "小人还在等图片上传完成"))
                await asyncio.sleep(0.5)
        ...
        douyin_logger.info(format_msg("📝", f"标题总字数: {title_len}，描述+话题总字数: {desc_and_tags_len}"))
        douyin_logger.info(format_msg("🏷️", f"小人一共贴了 {len(self.tags)} 个话题"))
        ...
        while True:
            try:
                ...
                douyin_logger.success(format_msg("🥳", "图文发布成功，小人开心收工"))
                break
            except Exception:
                douyin_logger.info(format_msg("🏃", "小人正在冲刺发布图文"))
                await asyncio.sleep(0.5)

    async def upload(self, playwright: Playwright) -> None:
        douyin_logger.info(format_msg("🧍", "小人先检查 cookie、图片和发布时间"))
        await self.validate_upload_args()
        douyin_logger.info(format_msg("🥳", "图文上传前检查通过"))
        ...
        try:
            page = await context.new_page()
            await page.goto("https://creator.douyin.com/creator-micro/content/upload", wait_until="domcontentloaded", timeout=90000)
            douyin_logger.info(format_msg("🧭", "小人正在赶往图文发布页"))
            await page.wait_for_url("https://creator.douyin.com/creator-micro/content/upload", timeout=90000)
