...
async def youtube_cookie_gen(account_file, headless: bool = False):
    ...
    async with async_playwright() as playwright:
        ...
        page = await context.new_page()
        await page.goto(STUDIO_URL, wait_until="domcontentloaded")
        youtube_logger.info(format_msg("🔐", "请在弹出的浏览器里登录 Google / YouTube 账号，登录后会自动保存"))
        ok = False
        ...
        if ok:
            ...
            youtube_logger.success(format_msg("✅", f"YouTube 登录态已保存: {account_file}"))
        else:
            youtube_logger.error(format_msg("😵", "等待登录超时，未保存登录态"))
        await browser.close()
        return build_login_result(ok, "logged_in" if ok else "timeout",
...
async def youtube_setup(account_file, handle: bool = False, return_detail: bool = False, headless: bool = False):
    ...
    if not Path(account_file).exists() or not await cookie_auth(account_file):
        if not handle:
            result = build_login_result(False, "cookie_invalid", "登录态不存在或已失效", account_file)
            return result if return_detail else False
        youtube_logger.info(format_msg("🥹", "YouTube 登录态不存在或失效，准备打开浏览器登录"))
        result = await youtube_cookie_gen(account_file, headless=headless)
        return result if return_detail else result["success"]
    result = build_login_result(True, "cookie_valid", "登录态有效", account_file)
    ...
    for _ in range(max_polls):
        ...
        if txt:
            if any(k in txt for k in ("处理", "检查", "上传完成", "已上传", "Processing", "complete", "Checks", "Finished")):
                youtube_logger.info(format_msg("✅", f"上传完成: {txt[:40]}"))
                return True
            if txt != last:
                youtube_logger.info(format_msg("⏳", f"上传中: {txt[:40]}"))
                last = txt
        await page.wait_for_timeout(5000)
    youtube_logger.warning(format_msg("⚠️", "等上传超时(30min)，仍尝试发布"))
    return False
...
class YouTubeVideo(BaseVideoUploader):
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        youtube_logger.info(format_msg("🎬", f"开始上传: {Path(self.file_path).name}"))
        ...
        # 3) 标题
        youtube_logger.info(format_msg("✍️", "填写标题"))
        await _fill_editable(page, "#title-textarea #textbox", self.title[:100])

        # 4) 简介
        if self.description.strip():
            youtube_logger.info(format_msg("✍️", "填写简介"))
        ...
        if self.thumbnail_path and Path(self.thumbnail_path).exists():
            try:
                ...
                youtube_logger.info(format_msg("🖼️", "封面已上传"))
            except Exception as exc:
                youtube_logger.warning(format_msg("⚠️", f"封面上传跳过（不影响发布）: {exc}"))
        ...
        if self.playlist:
            try:
                ...
                youtube_logger.warning(format_msg("⚠️", f"播放列表处理跳过（不影响发布）: {exc}"))
            finally:
        ...
        if self.tags:
            try:
                ...
                youtube_logger.warning(format_msg("⚠️", f"标签填写跳过（不影响发布）: {exc}"))
        ...
        # 10) 可见性
        youtube_logger.info(format_msg("🌐", f"设置可见性 = {self.visibility}"))
        ...
        youtube_logger.info(format_msg("📤", "等待上传完成（传完才发布）…"))
        await _wait_upload_complete(page)

        # 11) 发布
        await page.wait_for_timeout(1200)
        if not await _click_if_present(page, "#done-button", 15000):
            youtube_logger.warning(format_msg("🤔", "未找到发布按钮，可能上传未到可发布进度；请在窗口里手动发布"))
        else:
            ...
            youtube_logger.success(format_msg("🥳", f"发布完成（{self.visibility}）{(' ' + video_url) if video_url else ''}"))
