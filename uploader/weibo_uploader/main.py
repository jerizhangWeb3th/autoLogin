...
async def _grab_qr(page: Page, account_file: str) -> dict:
    ...
    for sel in selectors:
        ...
        if await loc.count():
            qr = loc
            weibo_logger.info(format_msg("🔍", f"找到二维码元素: {sel}"))
            break

    if not qr:
        # 最后兜底：截取整个页面中心区域
        weibo_logger.warning(format_msg("⚠️", "未找到二维码元素，截取页面截图"))
        ...
        weibo_logger.info(format_msg("🖼️", f"页面截图已保存到: {qrcode_path}"))
    ...
    weibo_logger.info(format_msg("🖼️", f"二维码已保存到: {qrcode_path}"))
...
async def weibo_cookie_gen(account_file, qrcode_callback=None, poll_interval: int = 3, max_checks: int = 120, headless: bool = LOCAL_CHROME_HEADLESS):
    ...
    """
    account_file = resolve_account_file(account_file, "weibo_uploader")
    Path(account_file).parent.mkdir(parents=True, exist_ok=True)
    qrcode_path = None
    result = build_login_result(False, "failed", "微博登录失败", account_file)
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if headless:
                weibo_logger.info(format_msg("🧍", "无头登录中：二维码已存为图片，请用微博APP扫码"))
            else:
                weibo_logger.info(format_msg("🧍", "请在打开的浏览器中扫码登录微博"))
            ...
            weibo_logger.info(format_msg("🧍", "请扫码，正在耐心等待登录完成"))
            ...
            for _ in range(max_checks):
                ...
                if "passport" not in current_url and "weibo.com" in current_url:
                    weibo_logger.info(format_msg("🥳", f"扫码成功，跳转到: {current_url}"))
                    result = build_login_result(True, "success", "微博扫码登录成功", account_file, qrcode_info, current_url)
                    break
                # 检查 cookies 中是否出现 SUB（部分情况页面不跳转但 cookie 已写入）
                cookies = await context.cookies()
                if any(c.get("name") == "SUB" and c.get("value") for c in cookies):
                    weibo_logger.info(format_msg("🥳", f"扫码成功（检测到 SUB cookie），当前: {current_url}"))
                    result = build_login_result(True, "success", "微博扫码登录成功", account_file, qrcode_info, current_url)
                    break
                await page.wait_for_timeout(poll_interval * 1000)
            else:
                result = build_login_result(False, "timeout", "等待微博扫码登录超时", account_file, qrcode_info, page.url)

            if result["success"]:
                await asyncio.sleep(2)
                await context.storage_state(path=account_file)
                weibo_logger.success(format_msg("🥳", f"cookie 已保存: {account_file}"))
        except Exception as exc:
            result = build_login_result(False, "failed", str(exc), account_file, current_url=page.url if "page" in locals() else "")
        finally:
            if remove_qrcode_file(qrcode_path):
                weibo_logger.info(format_msg("🧹", f"临时二维码文件已清理: {qrcode_path}"))
            if not result["success"]:
                weibo_logger.error(format_msg("😢", f"登录失败: {result['message']}"))
            await context.close()
            await browser.close()
    return result
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if "newlogin" in page.url or "passport" in page.url:
                weibo_logger.info(format_msg("🥹", "cookie 已失效（跳转到登录页）"))
                return False

            # 检查是否有「登录」按钮（未登录态会显示）
            login_btn = page.get_by_text("登录", exact=True).first
            if await login_btn.count() and await login_btn.is_visible():
                weibo_logger.info(format_msg("🥹", "cookie 已失效（出现登录按钮）"))
                return False

            weibo_logger.success(format_msg("🥳", "cookie 有效"))
            return True
        except Exception as exc:
            weibo_logger.warning(format_msg("😵", f"cookie 校验出错，按失效处理: {exc}"))
            return False
        finally:
            await browser.close()
...
async def weibo_setup(account_file, handle=False, return_detail=False, qrcode_callback=None, headless: bool = LOCAL_CHROME_HEADLESS):
    ...
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        ...
        weibo_logger.info(format_msg("🥹", "cookie 文件不存在或已失效，自动打开浏览器请扫码登录"))
...
class WeiBoVideo(BaseVideoUploader):
    ...
    def __init__(
        ...
        self.tags = tags or []
        self.account_file = resolve_account_file(account_file, "weibo_uploader")
    ...
    async def upload(self, playwright: Playwright) -> None:
        weibo_logger.info(format_msg("🧍", "先检查 cookie 和视频文件"))
        await self.validate_upload_args()
        weibo_logger.info(format_msg("🥳", "上传前检查通过"))
        ...
        try:
            ...
            weibo_logger.info(format_msg("🏃", f"开始上传视频: {self.title}"))
            ...
            weibo_logger.success(format_msg("🥳", "cookie 更新完毕"))
        finally:
            await context.close()
            await browser.close()
    ...
    async def _open_video_publish_page(self, page: Page) -> Page:
        ...
        weibo_logger.info(format_msg("🏃", "已打开视频发布页"))
        return publish_page
    ...
    async def _upload_video_file(self, page: Page) -> None:
        ...
        weibo_logger.info(format_msg("🏃", f"已选择视频文件: {self.file_path}"))
        ...
        while True:
            ...
            try:
                if await done.is_visible():
                    weibo_logger.success(format_msg("🥳", "视频上传完毕（'上传完成' 可见）"))
                    return
            except Exception:
                pass
            ...
            if time.monotonic() - last_log > 5:
                try:
                    if await uploading.is_visible():
                        ...
                        if m:
                            weibo_logger.info(format_msg("🏃", f"上传中 {m.group(1)}/{m.group(2)}MB"))
                        else:
                            weibo_logger.info(format_msg("🏃", "上传中…"))
                except Exception:
                    pass
    ...
    async def _fill_title(self, page: Page) -> None:
        ...
        weibo_logger.info(format_msg("🏷️", f"标题已填写: {title}"))
    ...
    async def _upload_thumbnail(self, page: Page) -> None:
        ...
        weibo_logger.info(format_msg("🏃", f"已选择封面图片: {self.thumbnail_path}"))
        ...
        try:
            ...
            weibo_logger.warning(format_msg("⚠️", "cropper 未见 blob 图，仍尝试点完成"))
        ...
        try:
            ...
            weibo_logger.warning(format_msg("⚠️", "封面关闭后主表单未确认可见，继续尝试"))
        weibo_logger.success(format_msg("🖼️", "封面已上传并完成"))
    ...
    async def _select_type(self, page: Page) -> None:
        ...
        weibo_logger.info(format_msg("🏷️", "类型已选：二创"))
    ...
    async def _select_declaration(self, page: Page) -> None:
        ...
        if not await ai_opt.locator('[class*="_checkActive"]').count():
            weibo_logger.warning(format_msg("⚠️", "内容声明「含AI生成内容」疑似未激活，仍尝试点确定"))
        ...
        if await confirm.count():
            await confirm.click()
            await page.wait_for_timeout(500)
        weibo_logger.info(format_msg("🏷️", "内容声明已选：含AI生成内容"))
    ...
    async def _fill_description(self, page: Page) -> None:
        ...
        if not await desc_field.count():
            weibo_logger.warning(format_msg("⚠️", "未找到描述输入框"))
            return
        ...
        if content:
            ...
            weibo_logger.info(format_msg("📝", f"描述已填写（{len(content)}字）"))
    ...
    async def _apply_collection(self, page: Page) -> None:
        ...
        try:
            ...
            weibo_logger.warning(format_msg("⚠️", f"打开合集开关异常，仍尝试找面板: {exc}"))
        ...
        try:
            ...
            weibo_logger.warning(format_msg("⚠️", "未见合集面板，跳过合集"))
            return
        ...
        for i in range(n):
            ...
            if name and name == target:
                ...
                matched = True
                weibo_logger.info(format_msg("🥳", f"已选已有合集：{target}"))
                break
            ...
            try:
                ...
                for _ in range(3):
                    ...
                    if await new_inp.count() and await new_inp.is_visible():
                        ...
                        break
                if not created:
                    weibo_logger.warning(format_msg("⚠️", f"「新建合集」未出现输入行，跳过合集继续发布：{target[:12]}"))
                    return
                await new_inp.click()
                await new_inp.fill(target[:12])
                await page.wait_for_timeout(500)
                weibo_logger.info(format_msg("🥳", f"已新建合集：{target[:12]}"))
            except Exception as exc:
                weibo_logger.warning(format_msg("⚠️", f"新建合集失败，跳过合集继续发布：{exc}"))
                return
    ...
    async def _submit_publish(self, page: Page) -> None:
        ...
        weibo_logger.info(format_msg("🏃", "已点击发布按钮(JS)"))
        ...
        while time.monotonic() - start < 60:
            try:
                if await again_btn.is_visible():
                    weibo_logger.success(format_msg("🥳", "视频发布成功（出现「再发一条视频」）"))
                    return
            except Exception:
                pass
            try:
                if await success_layer.is_visible():
                    weibo_logger.success(format_msg("🥳", "视频发布成功（成功层可见）"))
                    return
            except Exception:
                pass
            # 处理可能的二次确认对话框
                ...
                if await dialog.count() and await dialog.is_visible():
                    for name in ("确定", "确认", "继续", "仍然发布", "发布"):
                        ...
                        if await cb.count() and await cb.is_visible():
                            ...
                            weibo_logger.info(format_msg("🏃", f"已确认对话框：{name}"))
                            break
            except Exception:
                pass
