...
def main() -> None:
    ...
    sys.argv = ["publish.py", "--title", content["title"], "--desc", content["desc"], "--price", str(PRICE), card_img]
    # 注意：必须在独立 globals 中 exec，否则 publish.py 内部的
    # import（argparse 等）绑定到本函数局部命名空间，其 main() 无法解析 → NameError
    _pub_path = Path(__file__).parent / "publish.py"
