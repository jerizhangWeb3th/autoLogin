...
from uploader.base_login import build_launch_kwargs, build_login_result, emit_qrcode_callback, format_msg, resolve_account_file
...
async def cookie_auth(account_file):
    account_file = resolve_account_file(account_file, "tencent_uploader")
    async with async_playwright() as playwright:
        browser = await playwright.chromium.launch(**build_launch_kwargs(headless=True, channel="chrome" if not LOCAL_CHROME_PATH else None))
        try:
            ...
            try:
                ...
                tencent_logger.info(format_msg("🥹", "cookie 已失效（页面跳转到登录页），得重新登录一下"))
                return False
            except Exception:
                pass  # 8 秒内未跳转, 大概率已登录
            ...
            for fr in page.frames:
                if "open.weixin.qq.com/connect/qrconnect" in fr.url:
                    tencent_logger.info(format_msg("🥹", "cookie 已失效（页面出现扫码登录框），得重新登录一下"))
                    return False

            tencent_logger.success(format_msg("🥳", "cookie 有效"))
            return True
        except Exception as exc:
            tencent_logger.warning(format_msg("😵", f"cookie 校验时出错，按失效处理: {exc}"))
            return False
        finally:
            await browser.close()
...
async def _save_tencent_qrcode(page: Page, account_file: str, previous_qrcode_path: Path | None = None, qrcode_callback=None) -> dict:
    ...
    if previous_qrcode_path and previous_qrcode_path != qrcode_path:
        if qrcode_utils["remove_qrcode_file"](previous_qrcode_path):
            tencent_logger.info(format_msg("🧹", f"临时二维码文件已清理: {previous_qrcode_path}"))

    tencent_logger.info(format_msg("🖼️", f"二维码已经准备好啦，已保存到: {qrcode_path}"))
...
async def _wait_for_tencent_login(
    ...
    for _ in range(max_checks):
        if await _is_tencent_login_completed(page):
            tencent_logger.info(format_msg("🥳", f"扫码成功，已经跳转到登录后页面: {page.url}"))
            return build_login_result(True, "success", "视频号扫码登录成功", account_file, qrcode_info, page.url)

        if not scanned_logged and await _is_tencent_qrcode_scanned(page):
            tencent_logger.info(format_msg("📱", "已经扫码啦，还差手机端确认一下"))
            scanned_logged = True

        if await _is_tencent_qrcode_expired(page):
            tencent_logger.warning(format_msg("😵", "二维码失效了，小人马上去刷新"))
            ...
            try:
                ...
                tencent_logger.warning(format_msg("⚠️", f"刷新后未能重新提取二维码({exc})，请直接在浏览器窗口中扫码"))

        await asyncio.sleep(poll_interval)

    return build_login_result(False, "timeout", "等待视频号扫码登录超时", account_file, qrcode_info, page.url)
...
async def tencent_cookie_gen(
    ...
    async with async_playwright() as playwright:
        browser = await playwright.chromium.launch(**build_launch_kwargs(headless=headless, channel="chrome" if not LOCAL_CHROME_PATH else None))
        context = await browser.new_context()
        qrcode_path = None
        result = build_login_result(False, "failed", "视频号登录失败", account_file)
        try:
            page = await context.new_page()
            ...
            try:
                ...
                tencent_logger.warning(
                    format_msg("⚠️", f"提取二维码图片失败({exc})，请直接在弹出的浏览器窗口中扫码，登录流程不受影响")
                )
                qrcode_info = None
                qrcode_path = None
            tencent_logger.info(format_msg("🧍", "请扫码，小人正在耐心等待登录完成"))
            ...
            if qrcode_utils["remove_qrcode_file"](qrcode_path):
                tencent_logger.info(format_msg("🧹", f"临时二维码文件已清理: {qrcode_path}"))
            if not result["success"]:
                tencent_logger.error(format_msg("😢", f"登录失败: {result['message']}"))
            await context.close()
            await browser.close()
...
async def tencent_setup(
    ...
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        ...
        tencent_logger.info(format_msg("🥹", "cookie 失效了，准备打开浏览器重新登录"))
...
class TencentBaseUploader(BaseVideoUploader):
    def __init__(
        ...
        self.account_file = resolve_account_file(account_file, "tencent_uploader")
    ...
    async def wait_for_realtime_verification(
        ...
        tencent_logger.warning(format_msg("📱", f"需要管理员微信扫码完成实名验证: {output_path}"))
        ...
        tencent_logger.success(format_msg("🥳", "管理员实名验证已完成，继续发表"))
        return output_path
    ...
    async def upload_video_file(self, page: Page, file_path: str) -> None:
        ...
        if fi is None:
            ...
            try:
                ...
                tencent_logger.info(format_msg(
                    "📸",
    ...
    async def set_short_title(self, page: Page, title: str, short_title: str | None = None) -> None:
        ...
        if await field.count():
            ...
            tencent_logger.info(format_msg("🏷️", f"短标题已填写（{len(value)}字）：{value}"))
        else:
            tencent_logger.info(format_msg("🧾", "未找到短标题输入框，跳过短标题"))
    ...
    async def fill_title_and_tags(self, page: Page) -> None:
        ...
        tencent_logger.info(format_msg("🏷️", f"成功添加 hashtag: {len(self.tags)}"))

    async def fill_description(self, page: Page) -> None:
        await page.keyboard.press("Enter")
        await page.keyboard.type(self.desc)
        tencent_logger.info(format_msg("🏷️", f"成功添加 desc: {len(self.desc)}"))
    ...
    async def apply_collection(self, page: Page) -> None:
        ...
        try:
            ...
            if await trigger.count() == 0:
                tencent_logger.info(format_msg("🧾", "当前页面未发现「添加到合集」入口，跳过归集"))
                return
            ...
            await page.wait_for_timeout(500)
            tencent_logger.success(format_msg("🥳", f"已选择合集：{self.collection_name}"))
        except Exception as exc:
            tencent_logger.warning(format_msg("😵", f"选择合集失败，跳过归集继续发布: {exc}"))
            try:
                await page.keyboard.press("Escape")
            except Exception:
                pass
    ...
    async def apply_original_statement(self, page: Page) -> None:
        ...
        try:
            ...
            if not await entry.count():
                tencent_logger.info(format_msg("🧾", "当前页面未发现「视频标注」入口，跳过标注继续发布"))
                return
            await entry.click()
            await page.wait_for_timeout(800)
            ...
            await option.click()
            await page.wait_for_timeout(500)
            tencent_logger.success(format_msg("🏷️", f"视频标注已选择：{label_text}"))
        except Exception as exc:
            tencent_logger.warning(format_msg("😵", f"设置视频标注「{label_text}」失败，跳过继续发布：{exc}"))
        ...
        while True:
            if time.monotonic() > deadline:
                try:
                    ...
                    await page.screenshot(path=str(shot), full_page=True)
                    tencent_logger.error(format_msg("📸", f"上传超时现场已截图 {shot}"))
                except Exception:
                    pass
                raise RuntimeError(
            ...
            try:
                ...
                if await publish_button.count():
                    ...
                    if (
                        ...
                        tencent_logger.info(format_msg("🥳", "视频上传完毕"))
                        break
                ...
                if now - last_report >= 30:
                    ...
                    tencent_logger.info(format_msg("🏃", f"正在上传视频中...（已等 {waited} 秒）"))
                    last_report = now
                await asyncio.sleep(2)
                ...
                if upload_failed and delete_button:
                    ...
                    if retries > max_retries:
                        try:
                            ...
                            await page.screenshot(path=str(shot), full_page=True)
                            tencent_logger.error(format_msg("📸", f"上传反复失败，现场已截图 {shot}"))
                        except Exception:
                            pass
                        raise RuntimeError(
                    ...
                    tencent_logger.error(format_msg("😵", f"发现上传出错了，准备重试（第 {retries}/{max_retries} 次）"))
    ...
    async def submit_publish(self, page: Page) -> None:
        ...
        for wait_round in range(60):
            ...
            tencent_logger.warning(format_msg("😵", "60s 内未找到可见的发表/草稿按钮，尝试强制继续"))
        # 点发表/草稿
        for attempt in range(20):
            try:
                ...
                if is_draft:
                    ...
                    tencent_logger.success(format_msg("🥳", "视频草稿保存成功"))
                else:
                    ...
                    for _ in range(10):
                        ...
                        if "/post/create" not in cur:
                            tencent_logger.success(format_msg("🥳", "视频发布成功"))
                            return
                        if not await publish_btn.count():
                            tencent_logger.success(format_msg("🥳", "视频发布成功（按钮已消失）"))
                            return
                ...
                if attempt and attempt % 5 == 0:
                    tencent_logger.warning(format_msg("😵", f"发布仍未完成(第{attempt}次)，异常: {str(exc)[:60]}"))
                tencent_logger.info(format_msg("🏃", "视频正在发布中..."))
                await asyncio.sleep(1)
...
class TencentVideo(TencentBaseUploader):
    ...
    async def handle_upload_error(self, page: Page) -> None:
        tencent_logger.info(format_msg("😵", "视频出错了，重新上传中"))
    ...
    async def confirm_thumbnail_crop(self, page: Page) -> None:
        ...
        try:
            ...
            tencent_logger.warning(format_msg("😵", f"封面裁剪确认时出错，小人继续尝试保存主弹窗: {exc}"))
    ...
    async def set_single_thumbnail(
        ...
        if not cover_dialog:
            tencent_logger.info(format_msg("🧍", f"当前页面没有出现{label}封面编辑弹窗，小人先跳过"))
            return

        try:
            await self.upload_thumbnail_in_dialog(page, cover_dialog, thumbnail_path)
            tencent_logger.success(format_msg("🥳", f"{label}封面已经设置完成"))
        except Exception as exc:
            tencent_logger.warning(format_msg("😵", f"{label}封面设置失败，这次先跳过: {exc}"))

    async def set_thumbnail(self, page: Page) -> None:
        if not self.thumbnail_landscape_path and not self.thumbnail_portrait_path:
            return

        tencent_logger.info(format_msg("🖼️", "小人准备设置封面"))
    ...
    async def upload(self, playwright: Playwright) -> None:
        tencent_logger.info(format_msg("🧍", "小人先检查 cookie、视频文件和发布时间"))
        await self.validate_upload_args()
        tencent_logger.info(format_msg("🥳", "上传前检查通过"))

        browser = await playwright.chromium.launch(**build_launch_kwargs(headless=self.headless, channel="chrome" if not LOCAL_CHROME_PATH else None))
        context = await browser.new_context(storage_state=self.account_file)

        try:
            page = await context.new_page()
            await self.open_upload_page(page)
            tencent_logger.info(format_msg("🏃", f"小人开始搬运视频: {self.title}"))
            ...
            tencent_logger.success(format_msg("🥳", "cookie 更新完毕"))
        finally:
            await context.close()
            await browser.close()
...
class TencentNote(TencentBaseUploader):
    ...
    async def upload(self, playwright: Playwright) -> None:
        tencent_logger.info(format_msg("🧍", "小人先检查 cookie、图文图片和发布时间"))
        await self.validate_upload_args()
        tencent_logger.info(format_msg("🥳", "图文上传前检查通过"))
        ...
        try:
            page = await context.new_page()
            await self.open_upload_page(page)
            tencent_logger.info(format_msg("🏃", f"小人开始搬运图文，共 {len(self.image_paths)} 张图片"))
