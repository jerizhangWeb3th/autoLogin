...
async def _save_xhs_qrcode(
    ...
    if previous_qrcode_path and previous_qrcode_path != qrcode_path:
        if remove_qrcode_file(previous_qrcode_path):
            xiaohongshu_logger.info(format_msg("🧹", f"临时二维码文件已清理: {previous_qrcode_path}"))

    xiaohongshu_logger.info(format_msg("🖼️", f"二维码已经准备好啦，已保存到: {qrcode_path}"))
    qrcode_content = decode_qrcode_from_path(qrcode_path)
    if qrcode_content:
        print_terminal_qrcode(qrcode_content, qrcode_path, "小红书APP")
    else:
        xiaohongshu_logger.warning(format_msg("😵", f"终端没法完整显示二维码，请打开 {qrcode_path} 扫码"))
...
async def cookie_auth(account_file):
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if page.url.startswith(_build_xhs_creator_url("/login")):
                xiaohongshu_logger.info(format_msg("🥹", "cookie 已失效，得重新登录一下"))
                return False
            ...
            if await login_box.count():
                try:
                    if await login_box.is_visible():
                        xiaohongshu_logger.info(format_msg("🥹", "页面仍然停留在登录二维码页，按 cookie 失效处理"))
                        return False
                except Exception:
                    return False

            xiaohongshu_logger.success(format_msg("🥳", "cookie 有效"))
            return True
        except Exception as exc:
            xiaohongshu_logger.warning(format_msg("😵", f"cookie 校验时出错，按失效处理: {exc}"))
            return False
        finally:
            await browser.close()
...
async def xiaohongshu_setup(
    ...
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        ...
        xiaohongshu_logger.info(format_msg("🥹", "cookie 失效了，准备打开浏览器重新登录"))
...
async def xiaohongshu_cookie_gen(
    ...
    if headless:
        xiaohongshu_logger.info(format_msg("🖼️", "小红书登录将以无头模式运行，小人会输出终端二维码并保存本地二维码图片"))
    ...
    async with async_playwright() as playwright:
        ...
        qrcode_path = None
        qrcode_info = None
        result = build_login_result(False, "failed", "小红书登录失败", account_file)
        try:
            page = await context.new_page()
            ...
            xiaohongshu_logger.info(format_msg("🧍", "请扫码，小人正在耐心等待登录完成"))
            ...
            for _ in range(max_checks):
                if await _is_xhs_login_completed(page):
                    ...
                    if await cookie_auth(account_file):
                        xiaohongshu_logger.success(format_msg("🥳", "小红书扫码登录成功，小人开心收工"))
                        result = build_login_result(True, "success", "小红书扫码登录成功", account_file, qrcode_info, page.url)
                    else:
            ...
            if remove_qrcode_file(qrcode_path):
                xiaohongshu_logger.info(format_msg("🧹", f"临时二维码文件已清理: {qrcode_path}"))
            if not result["success"]:
                xiaohongshu_logger.error(format_msg("😢", f"登录失败: {result['message']}"))
            await context.close()
            await browser.close()
        return result
...
class XiaoHongShuBaseUploader(BaseVideoUploader):
    ...
    async def set_schedule_time_xiaohongshu(self, page: Page, publish_date: datetime):
        xiaohongshu_logger.info(format_msg("🕒", f"小人准备设置定时发布时间: {publish_date.strftime(self.date_format)}"))
    ...
    async def set_location(self, page: Page, location: str = "青岛市"):
        ...
        xiaohongshu_logger.info(format_msg("📍", f"小人准备设置位置: {location}"))
        ...
        try:
            ...
            xiaohongshu_logger.warning(format_msg("😵", "位置下拉列表没按预期出现，小人继续按旧逻辑查找"))
        await page.wait_for_timeout(1000)
        flexible_xpath = (
        ...
        try:
            ...
            xiaohongshu_logger.success(format_msg("🥳", f"位置已经设置成 {location}"))
            return True
        except Exception as e:
            xiaohongshu_logger.error(format_msg("😢", f"设置位置失败: {e}"))
            try:
                ...
                xiaohongshu_logger.debug(format_msg("🧍", f"位置下拉里一共找到 {len(all_options)} 个选项"))
                for i, option in enumerate(all_options[:3]):
                    option_text = await option.inner_text()
                    xiaohongshu_logger.debug(format_msg("🧾", f"候选位置 {i + 1}: {option_text.strip()[:50]}"))
            except Exception as inner_e:
                xiaohongshu_logger.debug(format_msg("😵", f"读取位置候选列表失败: {inner_e}"))
            return False
    ...
    async def fill_tags(self, page: Page) -> None:
        ...
        if len(self.tags) > max_tags:
            xiaohongshu_logger.warning(
                format_msg("🏷️", f"标签数量 {len(self.tags)} 超过小红书上限 {max_tags}，只取前 {max_tags} 个: {self.tags[:max_tags]}")
            )
        ...
        for tag in self.tags:  # 循环处理所有 tags
            ...
            try:
                ...
                xiaohongshu_logger.warning(
                    format_msg("🏷️", f"话题『{tag}』未出现候选，跳过该标签继续发布: {exc}")
                )
    ...
    async def check_original_declaration(self, page: Page) -> None:
        ...
        try:
            ...
            await page.wait_for_timeout(1000)
            xiaohongshu_logger.success(format_msg("🧾", f"来源转载已声明（来源：{source}）"))
        except Exception as exc:
            xiaohongshu_logger.warning(format_msg("⚠️", f"设置来源转载失败，跳过继续发布: {exc}"))
            try:
...
class XiaoHongShuVideo(XiaoHongShuBaseUploader):
    ...
    async def handle_upload_error(self, page: Page):
        xiaohongshu_logger.warning(format_msg("😵", "视频上传摔了一跤，小人马上重新上传"))
    ...
    async def set_thumbnail(self, page: Page, thumbnail_path: str):
        ...
        xiaohongshu_logger.info(format_msg("🖼️", "小人准备设置封面"))
        ...
        try:
            ...
            try:
                ...
                pass
            xiaohongshu_logger.success(format_msg("🥳", "封面已经设置完成"))
        except Exception as exc:
            xiaohongshu_logger.warning(format_msg("🖼️", f"封面设置失败，跳过该步骤继续发布（用视频首帧）：{exc}"))
            try:
                await page.keyboard.press("Escape")
                await page.wait_for_timeout(500)
            except Exception:
    ...
    async def upload_video_content(self, page: Page) -> None:
        xiaohongshu_logger.info(format_msg("🏃", f"小人开始搬运视频: {self.title}.mp4"))
        xiaohongshu_logger.info(format_msg("🧭", "小人正在赶往视频发布页"))
        ...
        while True:
            try:
                ...
                if preview_new:
                    ...
                    if upload_success:
                        xiaohongshu_logger.success(format_msg("🥳", "视频已经传完啦"))
                        break
                    
                    if self.debug:
                        normalized_text = all_text.strip().replace("\n", " ")
                        xiaohongshu_logger.debug(format_msg("🧍", f"预览区域内容: {normalized_text}"))
                    xiaohongshu_logger.debug(format_msg("🧍", "还没看到上传成功标识，小人继续等一会"))
                else:
                    # 尝试检查标题输入框是否已经出现，如果是，说明已经进入编辑状态
                    title_container = page.locator('input[placeholder*="填写标题"]')
                    if await title_container.count() > 0 and await title_container.is_visible():
                        xiaohongshu_logger.success(format_msg("🥳", "虽然没看到预览区，但标题框出来了，小人继续"))
                        break
                    xiaohongshu_logger.debug(format_msg("🧍", "还没拿到预览区域，小人继续等一会"))
            except Exception as e:
                xiaohongshu_logger.debug(format_msg("😵", f"上传状态还没稳定下来，小人继续观察: {e}"))
            await asyncio.sleep(2)

        xiaohongshu_logger.info(format_msg("✍️", "小人开始填标题、描述和话题"))
        ...
        while True:
            try:
                ...
                xiaohongshu_logger.success(format_msg("🥳", "视频发布成功，小人开心收工"))
                break
            except Exception:
                xiaohongshu_logger.info(format_msg("🏃", "小人正在冲刺发布视频"))
                if self.debug:
                    await page.screenshot(full_page=True)
                await asyncio.sleep(0.5)

    async def upload(self, playwright: Playwright) -> None:
        xiaohongshu_logger.info(format_msg("🧍", "小人先检查 cookie、视频文件、封面和发布时间"))
        await self.validate_upload_args()
        xiaohongshu_logger.info(format_msg("🥳", "上传前检查通过"))
        ...
        try:
            ...
            xiaohongshu_logger.success(format_msg("🥳", "cookie 更新完毕"))
        finally:
            await context.close()
            await browser.close()
...
class XiaoHongShuNote(XiaoHongShuBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        xiaohongshu_logger.info(format_msg("🏃", f"小人开始搬运图文，共 {len(self.image_paths)} 张图片"))
        xiaohongshu_logger.info(format_msg("🧭", "小人正在赶往图文发布页"))
        publish_url = _build_xhs_creator_url(
        ...
        xiaohongshu_logger.info(format_msg("📤", "小人正在上传图片"))
        ...
        while True:
            try:
                ...
                xiaohongshu_logger.success(format_msg("🥳", "图文素材已经传完，可以开始填写内容了"))
                break
            except Exception:
                xiaohongshu_logger.debug(format_msg("🧍", "图文素材还在上传，小人继续等一会"))
                await asyncio.sleep(1)
        ...
        while True:
            try:
                ...
                xiaohongshu_logger.success(format_msg("🥳", "图文发布成功，小人开心收工"))
                break
            except Exception:
                xiaohongshu_logger.info(format_msg("🏃", "小人正在冲刺发布图文"))
                if self.debug:
                    await page.screenshot(full_page=True)
                await asyncio.sleep(0.5)

    async def upload(self, playwright: Playwright) -> None:
        xiaohongshu_logger.info(format_msg("🧍", "小人先检查 cookie、图片和发布时间"))
        await self.validate_upload_args()
        xiaohongshu_logger.info(format_msg("🥳", "图文上传前检查通过"))
        browser = await playwright.chromium.launch(headless=self.headless, channel="chromium")
        context = await browser.new_context(
            permissions=["geolocation"],
            storage_state=self.account_file,
