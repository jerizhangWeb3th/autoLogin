...
async def _focus_desc_editor(page) -> None:
    ...
    for i, make in enumerate(strategies):
        try:
            ...
            if i > 0:
                kuaishou_logger.warning(format_msg(
...
def _print_ks_qrcode(qrcode_content: str, qrcode_path: Path) -> None:
    try:
        ...
        if "unexpected keyword argument 'compact'" not in str(exc):
            raise
        kuaishou_logger.warning(format_msg("😵", "检测到旧版二维码打印函数，小人切回兼容模式继续登录"))
...
async def _save_ks_qrcode(page: Page, account_file: str, previous_qrcode_path: Path | None = None, qrcode_callback=None) -> dict:
    ...
    if previous_qrcode_path and previous_qrcode_path != qrcode_path:
        if remove_qrcode_file(previous_qrcode_path):
            kuaishou_logger.info(format_msg("🧹", f"临时二维码文件已清理: {previous_qrcode_path}"))

    kuaishou_logger.info(format_msg("🖼️", f"二维码已经准备好啦，已保存到: {qrcode_path}"))
    qrcode_content = decode_qrcode_from_path(qrcode_path)
    if qrcode_content:
        _print_ks_qrcode(qrcode_content, qrcode_path)
    else:
        kuaishou_logger.warning(format_msg("😵", f"终端没法完整显示二维码，请打开 {qrcode_path} 扫码"))
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if "passport.kuaishou.com" in page.url:
                kuaishou_logger.info(format_msg("🥹", "cookie 已失效（跳到登录页）"))
                return False

            # 检查是否停留在介绍页（未登录状态显示"立即登录"按钮）
            login_btn = page.get_by_text("立即登录")
            if await login_btn.count() > 0:
                kuaishou_logger.info(format_msg("🥹", "cookie 已失效（介绍页）"))
                return False
            ...
            try:
                ...
                kuaishou_logger.success(format_msg("🥳", "cookie 有效"))
                return True
            except Exception:
                # 兜底：旧版检测（"机构服务"元素出现在未登录介绍页）
                if await _is_ks_cookie_invalid(page):
                    kuaishou_logger.info(format_msg("🥹", "cookie 已失效（机构服务页）"))
                    return False
                # 都没命中：保守判定为失效，避免假阳性
                kuaishou_logger.warning(format_msg("😵", "无法确认 cookie 有效性，按失效处理"))
                return False
        except Exception as exc:
            kuaishou_logger.warning(format_msg("😵", f"cookie 校验时出错，按失效处理: {exc}"))
            return False
        finally:
            await browser.close()
...
async def ks_setup(account_file, handle=False, return_detail=False, qrcode_callback=None, headless: bool = LOCAL_CHROME_HEADLESS, cdp_url: str | None = None):
    ...
    if not os.path.exists(account_file) or not await cookie_auth(account_file):
        ...
        kuaishou_logger.info(format_msg("🥹", "cookie 失效了，准备重新登录快手创作者平台"))
...
async def get_ks_cookie(
    ...
    if headless:
        kuaishou_logger.info(format_msg("🖼️", "快手登录将以无头模式运行，小人会输出终端二维码并保存本地二维码图片"))
    ...
    async with async_playwright() as playwright:
        ...
        qrcode_path = None
        qrcode_info = None
        result = build_login_result(False, "failed", "快手登录失败", account_file)
        try:
            page = await context.new_page()
            await page.goto(KUAISHOU_LOGIN_URL)
            kuaishou_logger.info(format_msg("🧍", "请在浏览器里扫码登录快手，小人正在耐心等待"))
            ...
            for _ in range(max_checks):
                if page.url.startswith(KUAISHOU_UPLOAD_URL) or await _is_ks_login_page_gone(page):
                    ...
                    if await cookie_auth(account_file):
                        kuaishou_logger.success(format_msg("🥳", "快手扫码登录成功，小人开心收工"))
                        result = build_login_result(True, "success", "快手扫码登录成功", account_file, qrcode_info, page.url)
                    else:
                        kuaishou_logger.error(format_msg("😢", "快手扫码完成了，但 cookie 校验失败"))
                ...
                if qrcode_info and await _is_ks_qrcode_expired(page):
                    kuaishou_logger.warning(format_msg("😵", "二维码失效了，小人马上去刷新"))
            ...
            if remove_qrcode_file(qrcode_path):
                kuaishou_logger.info(format_msg("🧹", f"临时二维码文件已清理: {qrcode_path}"))
            if not result["success"]:
                kuaishou_logger.error(format_msg("😢", f"登录失败: {result['message']}"))
...
class KSBaseUploader(BaseVideoUploader):
    ...
    async def set_schedule_time(self, page: Page, publish_date: datetime):
        kuaishou_logger.info(format_msg("🕒", "小人准备设置定时发布时间"))
...
class KSVideo(KSBaseUploader):
    ...
    async def apply_collection(self, page: Page) -> None:
        ...
        try:
            ...
            if await trigger.count() == 0:
                kuaishou_logger.warning(format_msg("😵", "未找到\"加入合集\"下拉框，跳过归集"))
                return
            ...
            kuaishou_logger.success(format_msg("🥳", f"已选择合集：{self.collection_name}"))
        except Exception as exc:
            kuaishou_logger.warning(format_msg("😵", f"选择合集失败，跳过归集继续发布: {exc}"))
            try:
                await page.keyboard.press("Escape")
            except Exception:
                pass
    ...
    async def handle_upload_error(self, page: Page):
        kuaishou_logger.warning(format_msg("😵", "视频上传摔了一跤，小人马上重新上传"))
    ...
    async def set_thumbnail(self, page: Page):
        ...
        kuaishou_logger.info(format_msg("🖼️", "小人准备设置封面"))
        ...
        kuaishou_logger.success(format_msg("🥳", "封面已经设置完成"))

    async def upload(self, playwright: Playwright) -> None:
        kuaishou_logger.info(format_msg("🧍", "小人先检查 cookie、视频文件、封面和发布时间"))
        await self.validate_upload_args()
        kuaishou_logger.info(format_msg("🥳", "上传前检查通过"))
        ...
        try:
            page = await context.new_page()
            await page.goto(KUAISHOU_UPLOAD_URL)
            kuaishou_logger.info(format_msg("🏃", f"小人开始搬运视频: {self.title}.mp4"))
            kuaishou_logger.info(format_msg("🧭", "小人正在赶往快手上传主页"))
            ...
            kuaishou_logger.info(format_msg("✍️", "小人开始填描述和话题"))
            ...
            for index, tag in enumerate(self.tags[:3], start=1):
                kuaishou_logger.info(format_msg("🏷️", f"小人正在添加第 {index} 个话题: #{tag}"))
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if number == 0:
                        kuaishou_logger.success(format_msg("🥳", "视频已经传完啦"))
                        break

                    if retry_count % 5 == 0:
                        kuaishou_logger.info(format_msg("🏃", "小人正在努力上传视频"))
                    ...
                    kuaishou_logger.warning(format_msg("😵", f"检查上传状态时出错，小人继续重试: {exc}"))
                    await asyncio.sleep(2)
                retry_count += 1

            if retry_count == max_retries:
                kuaishou_logger.warning(format_msg("😵", "超过最大重试次数，视频上传可能未完成"))
            ...
            while True:
                try:
                    ...
                    kuaishou_logger.success(format_msg("🥳", "视频发布成功，小人开心收工"))
                    break
                except Exception as exc:
                    kuaishou_logger.info(format_msg("🏃", f"小人正在冲刺发布视频: {exc}"))
                    if self.debug:
            ...
            if upload_success:
                ...
                kuaishou_logger.success(format_msg("🥳", "cookie 更新完毕"))
                await asyncio.sleep(2)
            await context.close()
            await browser.close()
...
class KSNote(KSBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        kuaishou_logger.info(format_msg("🏃", f"小人开始搬运图文，共 {len(self.image_paths)} 张图片"))
        kuaishou_logger.info(format_msg("🔀", "小人正在切换到图文发布"))
        await page.locator('div[role="tablist"] div[role="tab"]:has-text("图文")').click()
        await page.wait_for_timeout(1000)

        kuaishou_logger.info(format_msg("📤", "小人正在上传图片"))
        ...
        kuaishou_logger.info(format_msg("✍️", "小人开始填写图文内容和话题"))
        await _focus_desc_editor(page)
        await page.keyboard.press("Backspace")
        await page.keyboard.press("Control+KeyA")
        await page.keyboard.press("Delete")
        ...
        while retry_count < max_retries:
            try:
                number = await page.locator("text=上传中").count()
                if number == 0:
                    kuaishou_logger.success(format_msg("🥳", "图文素材已经传完啦"))
                    break

                if retry_count % 5 == 0:
                    kuaishou_logger.info(format_msg("🏃", "小人正在努力上传图文素材"))

                if await page.locator("text=上传失败").count():
                    kuaishou_logger.warning(format_msg("😵", "图文素材上传摔了一跤，小人马上重新上传"))
                    await page.locator('div.progress-div [class^="upload-btn-input"]').set_input_files(self.image_paths)

                await asyncio.sleep(2)
            except Exception as exc:
                kuaishou_logger.warning(format_msg("😵", f"检查图文上传状态时出错，小人继续重试: {exc}"))
                await asyncio.sleep(2)
            retry_count += 1

        if retry_count == max_retries:
            kuaishou_logger.warning(format_msg("😵", "超过最大重试次数，图文上传可能未完成"))
        ...
        while True:
            try:
                ...
                await page.wait_for_url(KUAISHOU_MANAGE_URL_PATTERN, timeout=5000)
                kuaishou_logger.success(format_msg("🥳", "图文发布成功，小人开心收工"))
                break
            except Exception as exc:
                kuaishou_logger.info(format_msg("🏃", f"小人正在冲刺发布图文: {exc}"))
                if self.debug:
                    await page.screenshot(full_page=True)
                await asyncio.sleep(1)

    async def upload(self, playwright: Playwright) -> None:
        kuaishou_logger.info(format_msg("🧍", "小人先检查 cookie、图片和发布时间"))
        await self.validate_upload_args()
        kuaishou_logger.info(format_msg("🥳", "图文上传前检查通过"))
        ...
        try:
            page = await context.new_page()
            await page.goto(KUAISHOU_UPLOAD_URL)
            kuaishou_logger.info(format_msg("🧭", "小人正在赶往快手图文发布页"))
            await page.wait_for_url(KUAISHOU_UPLOAD_URL_PATTERN)
